...
async function loadData() {
    try {
        ...
        try {
            const localResponse = await fetch('spojene_promjene_cijena_dionica.json');
            allData = await localResponse.json();
            filteredData = [...allData];
            displayData(filteredData);
            updateFilterStatus();
        } catch (localError) {
            console.error('Greška pri učitavanju lokalnih podataka:', localError);
            document.getElementById('tableBody').innerHTML = '<tr><td colspan="11">Greška pri učitavanju podataka</td></tr>';
        }
    }
}
